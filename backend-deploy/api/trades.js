...
router.post('/accept-listing/:id', async (req, res) => {
    try {
        ...
        try {
            ...
            // Record in trade history (PostgreSQL needs RETURNING)
...
router.post('/counter-offer/:listingId', async (req, res) => {
    try {
        ...
        // Create counter-offer (PostgreSQL needs RETURNING)
        const result = await run(`
...
router.post('/sell-card', async (req, res) => {
    try {
        ...
        // Get card with current market value (match by name AND variant)
        const card = await get(`
            SELECT ci.*, cmv.current_market_value
            FROM card_instances ci
            JOIN card_market_values cmv ON ci.card_name = cmv.card_name 
                AND COALESCE(ci.art_variant, 'standard') = COALESCE(cmv.variant, 'standard')
            WHERE ci.id = ?
        `, [cardInstanceId]);
        ...
        const marketValue = card.current_market_value; // Already includes 5x for Full Art from DB
        
        // Seller gets 25% of market value
        const sellPrice = Math.floor(marketValue * 0.25);
        
        // Store will list for 85% of market value (already includes 5x for Full Art)
