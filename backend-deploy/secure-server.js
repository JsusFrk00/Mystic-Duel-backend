...
io.on('connection', (socket) => {
    ...
    socket.on('gameAction', (data) => {
        // Don't log full data object (contains 30-card deck array!)
        const deckInfo = data.action?.deck ? `${data.action.deck.length} cards` : 'no deck';
        console.log('🎮 Game action from', socket.id, '- Action:', data.action?.type, 'Deck:', deckInfo);

        if (data.action?.type === 'deckSelected') {
            ...
            activeGames.forEach((game, gameId) => {
                if (game.player1.id === socket.id || game.player2.id === socket.id) {
                    ...
                    const deckSize = Array.isArray(data.action.deck) ? data.action.deck.length : 'unknown';

                    if (isPlayer1) {
                        game.player1.deck = data.action.deck;
                        game.player1.ready = true;
                        console.log('  ✅ Player 1 deck selected:', deckSize, 'cards');
                    } else {
                        game.player2.deck = data.action.deck;
